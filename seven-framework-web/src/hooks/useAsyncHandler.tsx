...
export type AsyncHandlerFunc<T, TArgs extends unknown[] = unknown[]> = (...args: TArgs) => Promise<T>;
export type AsyncHandlerErrorFunc = (error: unknown) => void;
export type AsyncHandlerVoidFunc = () => void;

type UseAsyncHandlerCallback<T> = <TArgs extends unknown[]>(
    action: AsyncHandlerFunc<T, TArgs>,
    actionParams: TArgs,
 ...
 * This hook also allows specifying a cleanup function to be executed when the components unmounts.
 *
 ...
 *                   Optional parameter, a cleanup function to be executed when the components unmounts.
...
const useAsyncHandler = <T,>(): [
  ...
  const run: UseAsyncHandlerCallback<T> = useCallback(async <TArgs extends unknown[]>(
    action: AsyncHandlerFunc<T, TArgs>,
    actionParams: TArgs,
    onError: AsyncHandlerErrorFunc = (error) => console.error(error),
    onCleanup?: AsyncHandlerVoidFunc,
  ) => {
